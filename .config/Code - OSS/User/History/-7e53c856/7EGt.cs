...
class Program
{
    ...
    static void Main()
    {
        while (true)
        {
            ...
            Console.WriteLine($"Товаров на складе: {_itemsInStock}");
            Console.WriteLine("1 - Показать остаток");
            Console.WriteLine("2 - Купить товар (Покупатель)");
    ...
    static void CustomerProcess(int count)
    {
        ...
        Console.WriteLine($"[Касса] Обслуживание клиента {id}...");
        ...
        if (_itemsInStock >= count)
        {
            Thread.Sleep(1500);
            _itemsInStock -= count;
            Console.WriteLine($"[Касса] Клиент {id} купил {count} шт. Осталось: {_itemsInStock}");
        }
        else
        {
            Console.WriteLine($"[Касса] Ошибка: Клиенту {id} не хватило товара (нужно {count}, есть {_itemsInStock})");
        }
        _stockMutex.ReleaseMutex();

        Console.WriteLine($"[Касса] Клиент {id} ушел.");
