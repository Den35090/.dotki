...
class Program
{
    ...
    // Массив мьютексов — по одному на каждый счет
    ...
    static void Main()
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("=== БАНКОМАТ (MUTEX PER ACCOUNT) ===");
            Console.WriteLine("1 - Показать балансы");
            Console.WriteLine("2 - Снять деньги (Поток)");
            Console.WriteLine("3 - Перевести деньги (Потоки)");
    ...
    static void ShowBalances()
    {
        ...
        {
            // Захватываем мьютекс, чтобы вывести актуальное состояние
    ...
    {
        Console.Write("Введите номер счета (0-4): ");
    ...
    {
        Console.WriteLine($"\n[Поток {Thread.CurrentThread.ManagedThreadId}] Ожидание доступа к счету #{acc}...");
        
        accountMutexes[acc].WaitOne(); // Блокируем только этот счет
        
        Console.WriteLine($"[Поток {Thread.CurrentThread.ManagedThreadId}] Проверка счета #{acc}...");
    ...
    static void TransferAction(int from, int to, decimal amount)
    {
        ...
        // Чтобы избежать Deadlock (взаимной блокировки), 
        // всегда захватываем мьютексы в порядке возрастания их индексов
        ...
        Thread.Sleep(2000); // Имитация задержки транзакции
