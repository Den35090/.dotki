...
if [[ "$1" = "init" ]]; then # choose first if dotswap is ran in init
  ...
  selection="$(echo "$pkglist" | rofi -dmenu -i -p "visual package" \
    -theme $HOME/.config/rofi/hyprmed.rasi 2> /dev/null)"
fi
...
stow --dir $STOWDIR --target $STOWTARGET --adopt --stow $selection
stow --dir $STOWDIR --target $STOWTARGET --adopt --restow $COREPKG

echo -e "\e[1;94mReloading\e[0m..."
"$RELOADSCRIPT" > /dev/null
# Добавь это в конец Switch.sh, перед выводом "Success"
openrgb --profile "$selection" --noautoconnect
pkill mako
mako &
